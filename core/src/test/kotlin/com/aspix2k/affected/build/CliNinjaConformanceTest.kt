...
class CliNinjaConformanceTest {

    @Test
    fun `ninja runs the project test command for the affected root`() = fixture("ninja") { root ->
        val module = ninjaRootModule(root)
        val command = ninjaCommands(root, listOf("${module.executionId}:${module.testTask}")).single()
        assertEquals(listOf("ninja", "test"), command.arguments)
