...
internal class GradleCollectorRun private constructor(
    ...
    private val artifacts: GradleCollectorArtifacts,
    val outputRoot: Path,
    ...
    private fun promote() {
        ...
        val store = DependencyMapStore(cacheRoot.resolve(MAPS_DIRECTORY))
    ...
    internal companion object {
        fun create(cacheRoot: Path, requestedArtifacts: GradleCollectorArtifacts): GradleCollectorRun? = runCatching {
            val root = secureDirectory(cacheRoot)
            val artifacts = requestedArtifacts.secure()
            ...
            GradleCollectorRun(root, artifacts, output, artifacts.version(), runId)
        }.getOrNull()

        private fun GradleCollectorArtifacts.secure(): GradleCollectorArtifacts = GradleCollectorArtifacts(
            agent.secureFile(),
            listener.secureFile(),
            initScript.secureFile(),
        )

        private fun GradleCollectorArtifacts.version(): String {
            val digest = MessageDigest.getInstance("SHA-256")
            listOf(agent, listener, initScript).forEach(digest::update)
...
private const val MAPS_DIRECTORY = "maps"
