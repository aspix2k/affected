...
class CrashSafetyTest {
    ...
    fun `план с пустыми путями не роняет планировщик`() {
        val plan = TaskPlanner.plan(
            listOf(ModuleInfo("", "", testTask = "test", compileTask = "compileTestKotlin", hasTests = true)),
            emptyList(),
        )
    ...
    fun `огромное число модулей не роняет планировщик`() {
        val modules = (1..5_000).map { ModuleInfo(":m$it", "/repo", testTask = if (it % 2 == 0) "testDebugUnitTest" else "test", compileTask = "compileTestKotlin", hasTests = true) }
    ...
    fun `дубли одинаковых модулей не размножают задачи`() {
        val one = ModuleInfo(":core", "/repo", testTask = "test", compileTask = "compileTestKotlin", hasTests = true)
